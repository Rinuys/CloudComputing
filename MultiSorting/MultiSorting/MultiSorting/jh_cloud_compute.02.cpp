...
void load_execute(){
	...
	for(long i=0;i<DATA_SIZE;i++){
		char buffer[14];
		fread(buffer,sizeof(char),12,rf);
		long cal=0l;
		for(int c=0;c<10;c++)
            cal = (cal << 6) + buffer[c] - '0';
        arr[i] = cal;

        if(i==end_value[next_thread]){
            pool[next_thread] = thread(myqsort,next_thread);
            next_thread++;   
        }
	}
	fclose(rf);
...
// void quick_sort(int id, long start, long end){
//     long* array=arr;
...
//     quick_sort(id,start,q-1);
//     quick_sort(id,q+1,end);
// }

// void myqsort(int id){
//     long start=start_value[id], end=end_value[id];
//     quick_sort(id,start,end);
//     finish_length[id]=end;    
// }